...
export default function QuotationDocumentActions({ quotation, lang, publicUrl }: { quotation: CustomerDocument; lang: "sw" | "en"; publicUrl?: string }) {
  const [busy, setBusy] = useState(false);
  const title = `${lang === "sw" ? "Nukuu ya Bei" : "Quotation"} ${quotation.quotationNumber}`;
  const markup = documentMarkup(quotation, lang);
  ...
  function whatsapp() { const url = publicUrl || window.location.href; const text = lang === "sw" ? `Habari, tafadhali angalia nukuu ya bei ${quotation.quotationNumber}: ${url}` : `Hello, please view quotation ${quotation.quotationNumber}: ${url}`; window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, "_blank", "noopener,noreferrer"); }
  ...
  return <div className="flex flex-wrap gap-2"><button type="button" onClick={() => { void pdf().catch(() => {}); }} disabled={busy} className={button} title="PDF"><>{busy ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4" />}</>PDF</button><button type="button" onClick={print} className={button} title={lang === "sw" ? "Chapisha" : "Print"}><Printer className="h-4 w-4" />{lang === "sw" ? "Chapisha" : "Print"}</button>{publicUrl && <><button type="button" onClick={() => { void copy().catch(() => {}); }} className={button} title={lang === "sw" ? "Nakili link" : "Copy link"}><Copy className="h-4 w-4" />{lang === "sw" ? "Nakili link" : "Copy link"}</button><button type="button" onClick={whatsapp} className={button} title="WhatsApp"><MessageCircle className="h-4 w-4 text-green-600" />WhatsApp</button></>}</div>;
}
