...
export default function HelpPage() {
  ...
  const faqs = [
    ...
    [lang === "sw" ? "Mfumo hufanya kazi bila intaneti?" : "Does offline work?", lang === "sw" ? "Ukiwa tayari umefungua Mauzo, unaweza kuhifadhi mauzo kwenye simu bila intaneti na kuyasawazisha intaneti ikirudi. Kwa shamba, Crop operations au Field plan iliyokwisha funguliwa inaweza pia kuhifadhi pembejeo, mavuno, umwagiliaji, task, grade, buyer commitment na weather observation kwa retry salama. Kufungua app upya bila mtandao bado kunahitaji intaneti; matumizi, madeni, stock count na Daily Close hubaki online." : "If the Sales page is already open, it can save sales locally while offline and sync them when internet returns. For farms, an already-open Crop operations or Field plan screen can safely retry inputs, harvests, irrigation, tasks, grades, buyer commitments, and weather observations. Reopening the app offline still requires a connection; expenses, debts, stock counts, and Daily Close remain online-only."],
    ...
    [lang === "sw" ? "Salio la Duka linafanyaje kazi?" : "How does Merchant Balance work?", lang === "sw" ? "Ikiwa Salio la Duka linaonekana kwenye menu yako, mmiliki pekee anaweza kuweka amana, kulipia usajili wa DukaPilot au kutoa pesa. Amana huongezwa baada ya mtandao kuthibitisha; utoaji unaonyesha makato ya DukaPilot na mtandao kabla ya uthibitisho. Malipo ya usajili hukatwa tu baada ya kuonyesha kiasi na kuthibitishwa na mmiliki. Salio hili si mauzo, si matumizi, wala cash ya Funga Siku. Usitume PIN kwenye DukaPilot." : "When Merchant Balance appears in your menu, only the owner can deposit, pay for a DukaPilot subscription, or withdraw. Deposits are credited only after provider confirmation; withdrawals show both DukaPilot and provider fees before confirmation. A subscription debit happens only after the owner sees and confirms the exact amount. This balance is not sales, an expense, or Daily Close cash. Never enter a mobile-money PIN in DukaPilot."],
    ...
    [lang === "sw" ? "Naweza kutumia DukaPilot kwa mazao?" : "Can I use DukaPilot for crops?", lang === "sw" ? "Ndiyo. Chagua Mazao na Ufugaji, kisha chagua Mazao au Vyote kwenye Farm. Ongeza plot, msimu wa zao na pembejeo, halafu rekodi mavuno moja au zaidi kwenye stock ya kuuza. Field plan huweka umwagiliaji, kazi, grades, buyer commitments na weather observations. Tumia bidhaa maalumu ya mavuno ili ripoti ya zao ibaki sahihi." : "Yes. Choose Crop & Livestock Farm, then choose Crops or Both in Farm. Add a plot, crop cycle, and inputs, then record one or more harvests into sellable stock. Field plan holds irrigation, tasks, grades, buyer commitments, and weather observations. Use a dedicated harvest product to keep crop reporting accurate."],
    [lang === "sw" ? "Nani anaona shift za Daily Close?" : "Who can see Daily Close shifts?", lang === "sw" ? "Cashier anaona na kufunga shift yake mwenyewe. Mmiliki anaona shift zote za timu na anaweza kufungua shift yake tofauti. Mmiliki anaweza kumpa manager ruhusa ya Kusimamia shift za timu ili akague au afunge drawer ya staff baada ya kuhesabu pesa; ruhusa hiyo haimpi Reports au faida ya biashara." : "A cashier sees and closes only their own shift. The owner sees every team shift and may open a separate one. The owner can give a manager Manage team shifts so they can review or close a staff drawer after counting cash; that permission does not grant Reports or business profit."],
