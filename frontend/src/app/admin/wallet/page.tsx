...
type AdminOverview = {
  ...
  retainedSubscriptionRevenueTzs: number;
...
export default function AdminWalletPage() {
  ...
  return <AppShell>
    <div className="mx-auto max-w-6xl space-y-5 pb-24 lg:pb-8">
      ...
      <section className="border border-gray-200 bg-gray-50 p-4 text-sm leading-6 text-gray-700"><div className="flex gap-3"><Scale className="mt-0.5 h-5 w-5 flex-none text-brand-700" /><p><strong className="text-gray-950">Reconciliation rule:</strong> settled provider balance should equal customer liability plus withdrawal fees and subscription revenue still retained in the pool. Pending payouts may not be debited yet, and a completed deposit may arrive before DukaPilot credits its ledger, so the provider balance can temporarily be as high as {formatTZS(overview?.provider.expectedRangeMaxTzs || 0)}. Investigate any balance outside that range before moving platform revenue.</p></div></section>
    </div>
  </AppShell>;
}
